'use client';
import React, { Fragment, useState, useEffect } from 'react'; // used useState and useEffect hook
import { Dialog, Transition } from '@headlessui/react';
import { XMarkIcon } from '@heroicons/react/24/outline';
import Link from 'next/link';
import { usePathname } from 'next/navigation'; //used usePathname
                    ...
                    <ul role="list" className='hidden md:flex space-x-8'>
                        {navigationMenu.map((menu, idx) => (
                            <li key={menu.label}>
                                ...
                                before:w-full before:h-[2px] before:bg-violet-600 
                                before:origin-[100%, 50%] before:transition-all 
                                before:duration-300 before:ease-in-out before:scale-x-0 
                                before:scale-y-[1] before:scale-z-[1] 
                                    ...
                                    <div className='flex h-full flex-col overflow-y-scroll bg-white shadow-xl'>
                                        <div className='flex-1 overflow-y-auto px-8 py-6 sm:px-12'>
                                            <div className='flex items-start justify-between'>
                                                <Dialog.Title className="text-lg font-medium text-grey-900">
                                                    Menu
                                                </Dialog.Title>
                                        ...
                                        <div className='border-t border-gray-200 px-4 py-6 sm:px-6'>
                                            <div>
                                                <Link href="/contact" className='block text-center text-[11.5px] tracking-[2px] font-bold uppercase bg-violet-600 py-4 px-5 text-white'>Contact us now</Link>
                                            </div>
                                        </div>
                                    </div>
                                </Dialog.Panel>
